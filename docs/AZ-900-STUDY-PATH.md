...
1. ~~Dominio 1 completo~~ — ya cubierto en la sesión de repaso (CapEx/OpEx, Shared Responsibility Model, economías de escala, tipos de nube, HA/escalabilidad/elasticidad/tolerancia a fallos). Falta solo profundizar "consistencia" como concepto independiente del SLA
2. **Lo poco que queda 📖 del Dominio 2**: Azure DNS, VPN Gateway/ExpressRoute, Azure Files/Disk Storage, matices de consistencia — 1 hora
3. **Lo poco que queda 📖 del Dominio 3**: Azure Mobile App (trivial) — ya se cubrió casi todo lo demás
